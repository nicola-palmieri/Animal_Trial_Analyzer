...
test_that("stratification fallback handles missing columns", {
  df <- tibble::tibble(
    subject_id = c("S1", "S2"),
    visit_day = c(0, 7),
    outcome = c(1.2, 2.3)
  ...
  res <- longitudinal_resolve_stratification(df, "arm")
  expect_null(res$var)
  expect_null(res$levels)
  expect_match(res$warning, "arm")
})
